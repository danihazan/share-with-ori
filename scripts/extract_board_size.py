...
def generate_sizes_file(folder_path):
    output_file = os.path.join(folder_path, 'board_sizes.txt')
    with open(output_file, 'w') as output:
        for filename in os.listdir(folder_path):
            if filename.endswith('.xsb'):
                file_path = os.path.join(folder_path, filename)
                rows, columns = get_board_size(file_path)
                output.write(f'{filename}: {rows} x {columns}\n')
...
print(f'Sizes of boards written to {os.path.join(folder_path, "board_sizes.txt")}')
